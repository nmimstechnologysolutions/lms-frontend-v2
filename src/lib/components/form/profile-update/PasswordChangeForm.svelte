<script lang="ts">
    ...
    export let formStage: "BASE" | "OTP" | "PASSWORD";
	...
	async function changePassword(isResend = false) {
		const { json, error } = await fetchApi<ApiResponse>(
			`${PUBLIC_AUTH_BASE_URL}/auth/reset-password`,
			'POST',{Oldpassword,password,confirmPassword}
		);
...
<InputGroup isRequired label="Current Password">
    <Input
        type="password"
        name="New Password"
